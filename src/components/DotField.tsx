...
const DotField = memo(function DotField({
  ...
  useEffect(() => {
    ...
    function buildDots(w: number, h: number) {
      ...
      const step = (p.dotRadius || 1.5) + (p.dotSpacing || 16);
    ...
    function tick() {
      ...
      const eng = Math.min(m.speed / 5, 1);

      ctx.clearRect(0, 0, w, h);

      const grad = ctx.createLinearGradient(0, 0, w, h);
      grad.addColorStop(0, p.gradientFrom || 'rgba(168, 85, 247, 0.35)');
      grad.addColorStop(1, p.gradientTo || 'rgba(99, 102, 241, 0.2)');
      ctx.fillStyle = grad;
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (distSq < crSq && eng > 0.01) {
          ...
          if (isBulge) {
            ...
            const push = factor * factor * (p.bulgeStrength || 67) * eng;
